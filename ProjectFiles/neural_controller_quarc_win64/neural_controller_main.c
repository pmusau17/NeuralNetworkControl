...
typedef RT_MODEL_neural_controller_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * neural_controller(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(neural_controller_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: neural_controller -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
